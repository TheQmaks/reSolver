...
/**
 * Detects Prosopo Procaptcha instances in HTTP response bodies.
 * Covers procaptcha widget class, data-sitekey attribute,
 * procaptcha.render() JS call, and js.prosopo.io script.
 */
public class ProcaptchaDetector implements CaptchaDetector {

    // Widget: class="procaptcha" ... data-sitekey="..."
    private static final Pattern WIDGET_PATTERN = Pattern.compile(
            "class=\"procaptcha\"[^>]*?data-sitekey=\"([^\"]+)\"",
            Pattern.CASE_INSENSITIVE
    );

    private static final Pattern WIDGET_REVERSED_PATTERN = Pattern.compile(
            "data-sitekey=\"([^\"]+)\"[^>]*?class=\"procaptcha\"",
            Pattern.CASE_INSENSITIVE
    );

    // JS API: procaptcha.render(el, { siteKey: '...' })
    private static final Pattern RENDER_CALL_PATTERN = Pattern.compile(
            "procaptcha\\.render\\([^,]*,\\s*\\{[^}]*?siteKey\\s*:\\s*['\"]([^'\"]+)",
            Pattern.DOTALL | Pattern.CASE_INSENSITIVE
    );

    // Script URL: js.prosopo.io
    private static final Pattern SCRIPT_PATTERN = Pattern.compile(
            "js\\.prosopo\\.io/js/procaptcha",
            Pattern.CASE_INSENSITIVE
    );
    ...
    public List<DetectedCaptcha> detect(String url, String responseBody) {
        ...
        if (results.isEmpty()) {
            ...
            if (scriptMatcher.find()) {
                ...
                results.add(new DetectedCaptcha(url, "procaptcha", "unknown", now, evidence));
            }
        }
    ...
    private static void addMatches(List<DetectedCaptcha> results, Set<String> seenKeys,
        ...
        while (matcher.find()) {
            ...
            if (seenKeys.add(siteKey)) {
                ...
                results.add(new DetectedCaptcha(url, "procaptcha", siteKey, now, evidence));
            }
        }
    }
